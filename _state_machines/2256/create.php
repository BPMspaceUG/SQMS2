...
foreach ($allRows["records"] as $row) {
    ...
    if (!$isCreateScript)
        api(['cmd'=>'makeTransition', 'param'=>['table'=>$tablename, 'row'=>[$priColname=>$ID, 'state_id'=>7492]]]);
    //--- Check if create, and already exists => set to select and exit
    if ($isCreateScript && $row[$fkcol1][$k1] == $myID1 && $row[$fkcol2][$k2] == $myID2) {
        // Set Row to selected
        api(['cmd'=>'makeTransition', 'param'=>['table'=>$tablename, 'row'=>[$priColname=>$ID, 'state_id'=>7491]]]);
        $allow = false;
    }
}
//-----------------------Output
